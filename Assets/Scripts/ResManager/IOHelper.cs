...
public static class IOHelper
{
    ...
    public static void SetData(string fileName, object pObject)
    {
        ...
        string fileName_buffdoc = fileName + "txt";
        File.Delete(fileName_buffdoc);
        StreamWriter streamWriter = File.CreateText(fileName_buffdoc);

        streamWriter.Write( toSave );

        streamWriter.Close();
        File.Delete(fileName);
        File.Move(fileName_buffdoc, fileName);
    ...
    {
        string data = IOHelper.LoadFlie(fileName);
        //对数据进行解密，32位解密密钥
    ...
    public static string LoadFlie(string filePath)
    {
        return File.ReadAllText(filePath);
    }
    public static string LoadJson(string filePath) //加载Json
    {
#if UNITY_ANDROID
        ...
        string jsondata = Encoding.UTF8.GetString(Encoding.UTF8.GetBytes(www.text));
        return jsondata;
#else
          return Encoding.UTF8.GetString(Encoding.UTF8.GetBytes(File.ReadAllText(filePath)));
#endif
    }
    //安卓有bug
    public static object GetData_nojiami(string fileName, Type pType)
    {
        string data = IOHelper.LoadJson(fileName); 
        //对数据进行解密，32位解密密钥
        // data = RijndaelDecrypt(data, "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx");
    ...
    private static object DeserializeObject(string pString, Type pType)
    {
        ...
        //使用Json.Net进行反序列化
        //NewBehaviourScript.print(" DeserializeObject 001" + pString);
        deserializedObject = JsonConvert.DeserializeObject(pString, pType);
         //deserializedObject =  JsonMapper.ToObject<keyConfigData>(pString);
