...
//				Debug.LogWarning("<Res not ref> name = " + name);
   ...
   //资源缓存列表
  // static Dictionary<string,UnityEngine.Object> m_Cache = new Dictionary<string, Object>();
   ...
   public static T Load<T>(string assetPath) where T:Object
   {
       ...
       {
           if(!ResourcesManager.Instance.IsResLoaded(assetPath))
           {
               throw new UnityException("res \""+assetPath+"\" not load over");
           }
           return (T)ResourcesManager.Instance.getResource(assetPath);
       }
   }
