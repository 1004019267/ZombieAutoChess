...
public class SyncBuffer_transform 
{
    #region Inspector
    /// <summary>
    ...
    [NonSerialized]
    public Vector3 ExtrapolationScaleDrift;
    /// <summary>
    /// Used to smooth out extrapolation error correction. You can force it to fully correct by setting this to 0.
    /// </summary>
    [NonSerialized]
    ...
    public virtual Vector3 Scale
    {
        get
        {
            return ScaleNoErrorCorrection + ExtrapolationScaleDrift;
        }
    }
    ...
    public virtual Vector3 PositionNoErrorCorrection
    {
        get
        {
            ...
            return _keyframes[0].Position + _keyframes[0].Velocity_ * _playbackTime;// + 0.5f * _keyframes[0].Acceleration * _playbackTime * _playbackTime;
        }
    }



    public virtual Vector3 ScaleNoErrorCorrection
    {
        get
        {
            if (!HasKeyframes)
            {
                Debug.LogWarning("Trying to access scale in an empty buffer. Zero vector returned.");
                return Vector3.zero;
            }

            if (DisableExtrapolation && IsExtrapolating)
                return _keyframes[0].Scale;

            // using physics formula "delta x = v0 * t + 0.5 * a * t * t"
            // https://www.khanacademy.org/science/physics/one-dimensional-motion/kinematic-formulas/a/what-are-the-kinematic-formulas
            return _keyframes[0].Scale + _keyframes[0].Velocity_Scale * _playbackTime;// + 0.5f * _keyframes[0].Acceleration * _playbackTime * _playbackTime;
        }
    }
    ...
    /// </summary>
    public virtual Vector3 Velocity_
    {
        get
        {
            if (!HasKeyframes)
            ...
            return _keyframes[0].Velocity_;// + _keyframes[0].Acceleration * _playbackTime;
        }
    }
    ...
    public virtual void AddKeyframe( float interpolationTime ,  Vector3 scale_in, Vector3 position, Quaternion rotation = default(Quaternion)  )
    {
        ...
        var scaleBeforeNewKeyframe = Scale;
        ...
        var calculatedVelocity_scale = interpolationTime > 0f ? (scale_in - lastReceivedKeyframe.Scale) / interpolationTime : Vector3.zero;
        ...
        var keyframe = new Keyframe
        {
            ...
            Velocity_ =  calculatedVelocity,

            Scale = scale_in,
            Velocity_Scale = calculatedVelocity_scale,
        ...
        lastReceivedKeyframe.Velocity_ = calculatedVelocity;
        ...
        var scaleAfterNewKeyframe    = ScaleNoErrorCorrection;

        ExtrapolationPositionDrift = positionBeforeNewKeyframe - positionAfterNewKeyframe;

        ExtrapolationScaleDrift = scaleBeforeNewKeyframe - scaleAfterNewKeyframe;
